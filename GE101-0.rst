...
.. HJB: a pedido do Fábio, incluir alguma informação sobre velocidades relativas... HJB: no capítulo de abertura do livro, colocar sobre a questão do uso das palavras (argumento Humpty-Dumpty) e sobre a questão da notação matemática.
...
Considere agora um segmento de reta `AB`, sobre uma reta suporte `r`, na qual estabelecemos uma orientação. Na verdade, podemos perceber que existem duas possíveis orientações no segmento `AB`: de `A` para `B` e de `B` para `A`. Para indicar essas possibilidades, utilizaremos flechas, como na figura abaixo.
...
Na :numref:`fig-geometria-operacoesvetores-02`, a orientação escolhida para o segmento da primeira reta foi de `A` para `B` e por isso representaremos o :index:`segmento orientado` por `\overrightarrow{AB}`. Já na segunda reta, o mesmo segmento de reta foi orientado de `B` para `A`, e, então, será denotado por `\overrightarrow{BA}`. Repare pelas notações adotadas para estes segmentos orientados que a ordem das letras que caracterizam os extremos fornecem a orientação adicionada ao segmento. 
...
Na :numref:`fig-geometria-operacoesvetores-03`, embora as flechas tenham sido desenhadas em lugares diferentes, todas elas possuem as mesmas características: módulo, direção e sentido. A uma coleção de flechas com as mesmas características, daremos o nome de *vetor*. Veja a próxima definição. 
...
Podemos representar um vetor de duas formas diferentes. A primeira delas é com uma letra minúscula e uma flecha, por exemplo `\vec{v}`. E, a outra é usando as duas letras que caracterizam as extremidades inicial e final de um de seus representantes, por exemplo `\overrightarrow{AB}`. Quando escrevemos `\vec{v}=\overrightarrow{AB}` estamos considerando que o segmento de reta orientado `AB` é um representante do vetor `\vec{v}`. 

Para indicar o módulo do vetor `\vec{v}` usaremos a notação `|\vec{v}|`. 

.. admonition:: Diferente terminologia adotada em alguns livros

   Alguns autores definem segmentos equipolentes como sendo segmentos orientados que possuem o mesmo módulo, direção e sentido. Usando essa terminologia, é possível definir vetores de maneira análoga a definição dada anteriormente. Neste livro, optamos por não trabalhar com a definição de equipolência.
...
Na :numref:`fig-geometria-operacoesvetores-03.1`, os vetores `\vec{u}` e `\vec{v}` são iguais, pois possuem o mesmo módulo, direção (estão sobre retas paralelas `r` e `s`) e sentido. Neste caso, usaremos a seguinte notação: `\vec{u}=\vec{v}`.
...
Vale a pena ressaltar que se ao comparar dois vetores, pelo menos uma das suas características forem diferentes, então diremos que os vetores são diferentes. Não é necessário que todos as características dos vetores sejam diferentes para que eles sejam diferentes.   
...
Na :numref:`fig-geometria-operacoesvetores-06`, o vetor `\vec{u}+\vec{v}` resultante da soma de `\vec{u}` com `\vec{v}` está sendo ilustrado. Note que `\overrightarrow{AB}` é um representante do vetor `\vec{u}` e `\overrightarrow{BC}` é um representante de `\vec{v}` estão justapostos, então, pela definição anterior, `\overrightarrow{AC}` é um representante de `\vec{u}+\vec{v}`.
...
.. figure:: _resources/SomaVetoresSimples.png
   ...
   Soma de dois vetores `\vec{u}` e `\vec{v}`.
   
Com esta definição, é possível somar dois vetores cujo representante do primeiro vetor possui extremidade final coincidente com a extremidade inicial do representante do segundo vetor, ou seja, quando os vetores são adjacentes. E caso isso não aconteça, é possível realizar essa operação? Sim, nesta situação basta escolher um outro representante do segundo vetor de forma que sua extremidade inicial coincida com a extremidade final do primeiro, e então aplicar a definição como no caso anterior.
...
Na :numref:`fig-geometria-operacoesvetores-07`, queremos somar os vetores `\vec{u}`, representado por `\overrightarrow{AB}`, e `\vec{v}`, representado por `\overrightarrow{CD}`. Como os representantes de `\vec{u}` e `\vec{v}` não estão justapostos, é necessário escolher um outro representante do vetor `\vec{v}` justaposto ao representante de `\vec{u}` e então aplicar a definição. Sendo `\overrightarrow{BP}` um representante de `\vec{v}` justaposto à `\overrightarrow{AB}`, o vetor com extremidade inicial em `A` e extremidade final em `P` é um representante do vetor soma `\vec{u}+\vec{v}`.

**Importante:** Na :numref:`fig-geometria-operacoesvetores-07` temos dois representantes do vetor `\vec{v}`, sendo eles os segmentos orientados `\overrightarrow{BP}` e `\overrightarrow{CD}`. Esses dois segmentos, por possuírem o mesmo módulo, direção e sentido, pertencem à mesma coleção e por isso dão origem ao mesmo vetor.
  
Em geral, os dois vetores a serem somados e o vetor soma formam um triângulo. Por causa disso, esse método é conhecido como *Regra do Triângulo*.
...
.. admonition:: Para o professor
   ...
   O objetivo da reflexão acima é fazer o aluno identificar que quando os vetores a serem somados possuem a mesma direção, então não é possível formar um triângulo. Note que neste caso, o triângulo não vai existir, mas a operação deverá ser executada seguindo a definição.
...
* Comutatividade da Soma: observando novamente a :numref:`fig-geometria-operacoesvetores-09`, podemos notar que ao traçar a diagonal do paralelogramo `ABDC`, dividimos o paralelogramo em dois triângulos: `ABD` e `ACD`. Repare que se considerarmos `\overrightarrow{AB}` e `\overrightarrow{BD}` representantes dos vetores `\vec{u}` e `\vec{v}`, respecitvamente, então, pela regra do triângulo, `\overrightarrow{AD}` é um representante do vetor  `\vec{u}+\vec{v}`. Agora, se `\overrightarrow{AC}` e `\overrightarrow{CD}` são representantes dos vetores `\vec{v}` e `\vec{u}`, respectivamente, então, \overrightarrow{AD}` é um representante do vetor `\vec{v}+\vec{u}`. Portanto, \overrightarrow{AD}` é um representante tanto de `\vec{u}+\vec{v}` quanto de `\vec{v}+\vec{u}`. Assim, podemos concluir que 
...
Considere agora um segmento de reta `AB`, sobre uma reta suporte `r`, na qual estabelecemos uma orientação. Na verdade, podemos perceber que existem duas possíveis orientações no segmento `AB`: de `A` para `B` e de `B` para `A`. Para indicar essas possibilidades, utilizaremos uma flecha, como na :numref:`fig-geometria-operacoesvetores-02`.  
...
.. figure:: _resources/SegmentosOrientados_2.png
   ...
   Segmento de reta orientados de `A` para `B` e de `B` para `A`.   
Na figura acima, mostrarmos duas orientações diferentes para o mesmo segmento. A orientação escolhida no primeiro exemplo foi de `A` para `B` e por isso representaremos o :index:`segmento orientado` por `\overrightarrow{AB}`. Já no segundo exemplo, o mesmo segmento foi orientado de `B` para `A` , e, então, será denotado por `\overrightarrow{BA}`. Repare pelas notações adotadas para estes segmentos orientados que a ordem das letras que caracterizam os extremos fornecem a orientação adicionada ao segmento. 
...
**Importante:** Na :numref:`fig-geometria-operacoesvetores-07` temos dois representantes do vetor `\vec{v}`, sendo eles os segmentos orientados `BP` e `CD`. Esses dois segmentos, por possuirem o mesmo módulo, direção e sentido, pertencem à mesma coleção e por isso dão origem ao mesmo vetor.
...
* Associatividade da soma: considere `\overrightarrow{AB}, \overrightarrow{BC}` e `\overrightarrow{CD}` representantes dos vetores `\vec{u}, \vec{v}` e `\vec{w}`, respectivamente, como na :numref:`fig-geometria-operacoesvetores-10`. 
...
Logo, `\overrightarrow{AC}` é um representante de `\vec{u} + \vec{v}` . Somando `\vec{u} + \vec{v}` com `\vec{w}` através de seus representantes `\overrightarrow{AC}` e `\overrightarrow{CD}`, respectivamente, obtemos o vetor soma `(\vec{u} + \vec{v})+\vec{w}` que possui `\overrightarrow{AD}` como representante.
...
Assim, `\overrightarrow{AD}` é um representante dos vetores `(\vec{u} + \vec{v})+\vec{w}` e `\vec{u} + (\vec{v}+\vec{w})`. Como vetores que possuem representantes como mesmo módulo, direção e sentido são iguais, podemos concluir que:
...
* Comutatividade da soma: Observemos novamente a :numref:`fig-geometria-operacoesvetores-09`. Considerando
`\overrightarrow{AB}` e `\overrightarrow{BD}` representantes dos vetores `\vec{u}` e `\vec{v}`, respectivamente, então, pela regra do triângulo,  temos que `\overrightarrow{AD}` é um representante do vetor soma `\vec{u}+\vec{v}`. 

Por outro lado, se `\overrightarrow{AC}` e `\overrightarrow{CD}` são os representantes dos vetores `\vec{v}` e `\vec{u}`, respectivamente, então, novamente pela regra do triângulo, `\overrightarrow{AD}` é um representante do vetor soma `\vec{v}+\vec{u}`. Portanto, `\vec{u}+\vec{v}` e `\vec{v}+\vec{u}` podem ser representados pelo mesmo segmento orientado `\overrightarrow{AD}`. Assim, podemos concluir que 
