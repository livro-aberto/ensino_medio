...
.. admonition:: Roteiro
   ...
   .. admonition:: Estrutura
      ...
      **Organizando as ideias.**
      ...
      #. Vetor geométrico. Matemática apenas. Definir vetores iguais, adição de vetores geométricos (comutatividade, paralelogramo e triângulo) e multiplicação por escalar usando a estrutura da reta numérica: dado um vetor `\overrightarrow{v}`, considere uma reta paralela a `\overrightarrow{v}` e um representante da classe de `\overrightarrow{v}` sobre a reta. Considere coordenadas nesta reta de modo a origem e a extremidade de `\overrightarrow{v}` correspondem a 0 e a 1, respectivamente. O vetor `\lambda\overrightarrow{v}` é definido como aquele que tem origem em 0 e extremidade no ponto `\lambda` da reta. Estabelecer os vetores simétrico e nulo.
      ...
      **Explorando - coordenadas de vetores.** 
      ...
      #. Um dos motivos para se ensinar coordenadas de vetores é para se obter facilmente algumas propriedades algébricas de operações de vetores através das coordenadas.
      #. Atividades utilizando malhas e não usando um ponto de referência (origem do sistema de coordenadas) para que o estudante, contando quadrados da malha, possa se expressar usando esquerda, direita, cima e baixo para identificar as direções. Usar apenas malhas retangulares, estabelecer coordenadas de vetores. **Precisa ser contextualizada?**
      ...
      #. Atividade: **Translação**. Computador entende apenas números. No que segue não poderemos utilizar a notação funcional pois o estudante não terá proeficiência neste aspecto neste momento. Apresentar uma translação através da equação vetorial, move o ponto `P` para o ponto `Q` tal que `\overrightarrow{OQ}=\overrightarrow{OP} + \overrightarrow{v}` e pedir para representar por coordenadas. Estabelecer visualmente que o vetor soma é representado pela soma das coordenadas. Talvez realizar passos intermediários com translação à direita e outra com translação para cima e pedir as coordenadas. Depois compor as translações para obter uma situação mais geral. Pedir inicialmente para calcular a posição final de um ponto. Relacionar com o deslocamento. Observação: A translação de um ponto não depende do referencial escolhido.
      ...
      **Organizando as ideias de coordenadas.**
      ...
      #. Fazer um fechamento a cerca das coordenadas e operações usando coordenadas. Rever a comutatividade da adição através das coordenadas.
      #. Apresentar vetores do ponto de vista geométrico no contexto de forças. Incluir a decomposição de forças num sistema de eixos coordenados. Enfatizar que a regra do paralelogramo pode ser mais intuitivo no 
