...
#. É possível marcar no mapa a localização do grupo de exploradores? Por quê?
...
**DESCREVENDO FLECHAS VERBALMENTE**

Suponha que o lado de cada quadradinho da malha quadriculada no mapa da :numref:`fig-geometria-cv-02` tenha 1 cm.  Assim, a flecha `\vec{a}` pode ser interpretada da seguinte maneira: "se desloce 1 cm para o norte". Como as demais flechas podem ser descritas?
...
Antes de definir vetor, é bom lembrar que um :index:`segmento de reta` é um conjunto de pontos sobre uma reta delimitado por dois pontos chamados *extremos*, ou seja, segmento de reta é apenas uma parte de uma reta. Na :numref:`fig-geometria-operacoesvetores-08` temos uma reta `r` e um segmento de reta, desenhado na cor laranja, que contém os pontos compreendidos entre `A` e `B`. Representamos este segmento de reta pelas duas letras que caracterizam seus pontos extremos, ou seja, o segmento de reta da cor laranja é chamado `AB` ou `BA`. Neste caso, a ordem escolhida pelas letras não é importante, pois interessados apenas em conhecer os pontos localizados entre os pontos extremos, sem estabelecer nenhuma relação de ordem. A reta `r`, da qual extraímos `AB`, é chamada de reta suporte de `AB`. 
...
.. figure:: _resources/segmentodereta3.jpg
   ...
   Segmento de reta `AB` [falta fazer no Geogebra].

Considere agora um segmento de reta `AB` sobre uma reta suporte `r`, como anteriormente, na qual estabelecemos uma orientação. Na verdade, podemos perceber que existem duas possíveis orientações no segmento `AB`: de `A` para `B` e de `B` para `A`. Para indicar essas possibilidades, vamos utilizar uma flecha sobre o segmento, como na :numref:`fig-geometria-operacoesvetores-09`. 
...
Na figura da esquerda, a orientação escolhida foi de `A` para `B` e por isso representaremos o segmento orientado por `AB`. Já na figura da direita, temos um segmento de reta orientado de `B` para `A`, que será denotado por `BA`. Note pelas notações destes segmentos orientados que a ordem das letras que caracterizam os extremos fornecem a orientação adicionada ao segmento.

Usaremos a mesma notação para segmento de reta e segmento de reta orientado, ou seja, as duas letras que caracterizam seus extremos. Para que não haja confusão, é preciso sempre fazer menção se o segmento é orientado ou não ao se trabalhar com este ente geométrico.

.. admonition:: Definição 
	   
   :index:`Vetor` é uma coleção de segmentos orientados de mesmo comprimento, direção e sentido.
