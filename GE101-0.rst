...
#. Se a estação de pesquisa `A` e os dois grupos de pesquisa `P` e `Q` estivem alinhados (isto é, pertencessem a uma reta), seria possível marcar a localização do grupo `Q` de exploradores? Justifique sua resposta!
...
Se um objeto se desloca de um ponto `A` para um ponto `B` e, em seguida, se desloca do ponto `B` de volta para o ponto `A`, qual é 
