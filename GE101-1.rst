...
Deste modo qualquer vetor do plano pode ser expresso em termos de `\overrightarrow{OX}` e `\overrightarrow{OY}` e também dados quaisquer dois números reais `x` e `y`, existe um vetor com a coordenadas `(x,y)`. Veja a figura.
