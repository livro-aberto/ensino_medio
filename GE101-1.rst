...
Nesta seção você vai aprender a representar vetores através de uma lista de números que serão chamados de coordenadas do vetor. Esta representação possibilita que os computadores entendam e operem com vetores. Também serão fundamentais para o estudo das transformações do plano.
...
Reproduza em seu caderno a malha quadriculada e os vetores `\vec{u}` e `\vec{v}` como na figura e faça o que se pede:
...
Escolhendo dois vetores de direções diferentes num plano, digamos `\overrightarrow{u}` e `\overrightarrow{v}`, podemos construir uma malha justapondo os vetores como na figura:

.. tikz:: Uma malha determinada por dois vetores de direções diferentes

	   \foreach \x in {0,.5,...,1.5} {
       \draw[-latex, thick, red] (\x,0) -- (\x+.5,0);
       \draw[-latex, thick, red] (\x,.5) -- (\x+.5,.5);
       \draw[-latex, thick, red] (\x,1) -- (\x+.5,1);
       \draw[-latex, thick, red] (\x,1.5) -- (\x+.5,1.5);
       \draw[-latex, thick, red] (\x,2) -- (\x+.5,2);
    } 
    \foreach \y in {0,.5,...,1.5} {
       \draw[-latex, thick, red] (0,\y) -- (0,\y+.5);
       \draw[-latex, thick, red] (.5,\y) -- (.5,\y+.5);
       \draw[-latex, thick, red] (1,\y) -- (1,\y+.5);
       \draw[-latex, thick, red] (1.5,\y) -- (1.5,\y+.5);
       \draw[-latex, thick, red] (2,\y) -- (2,\y+.5);
    }
    \node at (.25,-.25){$\vec{u}$};
    \node at (-.25,.25){$\vec{v}$};


Qualquer vetor que possua sua origem e sua extremidade em vértices da malha pode ser descrito por meio de operações com `\vec{u}` e `\vec{v}`. Por exemplo, na figura abaixo o vetor `\vec{w}=3\vec{u} + 4\vec{v}`.
