...
Então podemos dizer que a :index:`força <força>` é o que causa a *variação na velocidade*. 

Quando alguém empurra um caminhão e ele não se move, isto não significa que o Princípio da Inércia esteja errado. Significa que existe uma outra força que *anula* a força do empurrão, no caso, o *atrito* do caminhão com o chão.

**FIGURA** do caminhão sendo empurrado por uma pessoa com a representação vetorial da força da pessoa no caminhã e a força de atrito do chão no caminhão.

Uma maneira de expressar o Princípio da Inércia de modo a evitar confusões como esta da situação do caminhão é introduzir antes o conceito de *força resultante*. 
...
**FIGURA** - partícula com 3 forças atuando e a resultante de cor diferente.
...
Portanto, se quando empurramos um caminhão ele não se mexe é porque existem forças de resistência no caminhão que anulam a força com que empurramos o caminhão.

.. admonition:: Exemplo: Se não pedalar a bicicleta para

   Em uma ciclovia retilínea e horizontal um ciclista em movimento para de pedalar. A velocidade da bicicleta diminui até parar. Como a força exercida pelas pernas do ciclista pararam de empurrar a bicicleta, ela não devia permanecer em movimento retilíneo uniforme pelo Princípio da Inércia?
   
   Resposta:
   
.. admonition:: Exemplo: Força peso

   Força peso - Corpo em queda livre e objeto parado no chão. Objetivos específicos: mesmo que nada esteja em contato com o corpo solto no ar ele está em movimento acelerado (variando a velocidade), introduzir a força da gravidade. Apresentar a força normal que o chão aplica num corpo apoiado nele.


Atividade da escaladora: Resultante zero e soma de vetores com mesma origem.
...
A figura esboça um plano inclinado com um bloco sobre ele, a força peso, nele representada e a direção em que o movimento acontece tracejada. Para o que segue despreze o atrito do bloco com o plano.

**Figura**

#. Reproduza a figura em seu caderno e trace um sistema de eixos coordenados com origem em `O`, eixo `x` paralelo à direção do movimento e eixo `y` perpendicular ao eixo `x`.
#. Represente uma decomposição de `\vec{P}` como `\vec{P}=\vec{P_x} + \vec{P_y}` com `\vec{P_x}` e `\vec{P_y}` com origem em `O` e ambos paralelos aos respectivos eixos coordenados.
#. Represente a força normal `\vec{N}` que o plano exerce no bloco.
#. Como o movimento é retilíneo, o vetor resultante tem mesma direção do movimento. Use este fato para calcular `\vec{N} + \vec{P_y}`.
