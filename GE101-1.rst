...
.. tikz:: Dois vetores de direções diferentes determinam uma malha.
    ...
    \node at (6.5,1.5) {$\vec{w}=2\vec{u} + 3 \vec{v}$.};

Embora uma discussão mais geral seja útil para estudantes da área de exatas, nos restringiremos neste livro ao caso em que os vetores `\vec{u}` e `\vec{v}` têm módulos ambos iguais a 1 e são perpendiculares (isto significa que eles estão sobre retas perpendiculares).

Você deve lembrar dos anos anteriores como marcar pontos num plano com um sistema de coordenadas. Pretendemos relacionar este cconhecimento com o de vetores. Se a atividade a seguir parecer difícil, sugiro que realize antes as atividades de revisão deste link (**INSERIR LINK PARA ALGUMAS DAS ATIVIDADES ONLINE DO SESAMATH - TRADUZIDAS - DE INTRODUÇÃO DO SISTEMA DE COORDENADAS NO PLANO**).
...
Qualquer vetor `\vec{v}` do plano pode ser representado como possuindo origem no ponto `O=(0,0)`, do sistema de coordenadas de modo que `\vec{v}=\overrightarrow{OV}`, para algum ponto `V` do plano.  Na atividade acima você deve ter observado que as coordenadas do vetor posição `\overrightarrow{OV}` de um ponto `V=(x,y)` qualquer são também `x` e `y` (pois `\overrightarrow{OV}=x\overrightarrow{OX} + y\overrightarrow{OY}`). Deste modo, quando temos um sistema de coordenadas fixado escrevemos `\vec{v} = (x,y)`.
