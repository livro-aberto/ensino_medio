...
Veja na :ref:`sec-Parasabermais` a demonstração da propriedade de que a soma dos desvios da média é sempre nula. 
...
Quando lidamos com grande quantidade de dados, calcular a variância usando a definição apresentada será uma tarefa maçante, pois após calcular a média de muitos dados teremos que calcular cada desvio da média, elevá-los ao quadrado e, finalmente, somá-los. Se o conjunto tiver 50 valores será muito trabalhoso calcular a variância desta forma. Um modo menos trabalhoso de calcular a variância é calcular a média dos dados (`\bar{x}`) e a soma de quadrados dos dados, a saber, `\displaystyle{\sum^n_{i=1}x^2_i}`. De fato, é possível mostrar que

.. math::

   \textsf{variância} = \frac{1}{n}\cdot \left (\sum^n_{i=1} x^2_i-n\cdot \bar{x}^2\right )
