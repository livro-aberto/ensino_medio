...
As etapas da metodologia estatística podem ser divididas em duas estruturas básicas: a da **Estatística Descritiva** e da **Estatística Inferencial**. A primeira corresponde a uma exploração das informações possíveis de serem retiradas de dados amostrais a fim de se reconhecer estruturas que possibilitem futuramente inferir sobre os parâmetros populacionais de interesse; a segunda fase, denominada inferencial, consiste em estabelecer modelos probabilísticos para que se possa tecer afirmações sobre a população com uma certa confiabilidade estabelecida. Em resumo, a Estatística Descritiva é uma espécie de arqueologia dos dados observados e a Estatística Inferencial é a indução das informações obtidas da amostra para características de interesse da população não observada em sua totalidade.
...
.. admonition:: Proposições matemáticas versus proposições estatísticas
 ...
 Uma proposição é dita matemática se é possível estabelecer o valor *verdadeiro* ou *falso* a ela, ainda que essa afirmação seja uma conjectura não provada. Assim, a proposição *"O quadrado de um número par é par."*  é uma proposição matemática, pois sabemos que ela é verdadeira. Da mesma forma, a proposição *"O triângulo de lados 6, 4 e 3 é um triângulo retângulo."* é uma proposição matemática, pois sabemos que é falsa. 
 
 Por outro lado, uma proposição estatística é uma afirmação sobre a qual nunca teremos condição de afirmar se é *verdadeira* ou *falsa*, mas apenas aferir um grau de confiabilidade para ela. 
 
 A proposição *"Uma moeda, que ao ser lançada 10 vezes, resulta em 10 caras, é uma moeda viciada (as probabilidades de se obter cara e coroa não são iguais)."* é uma proposição estatística, pois existe a possibilidade de em 10 lançamentos da moeda obtermos 10 coroas, embora isso seja pouco provável de ocorrer numa moeda não-viciada (probabilidades de obter cara e coroa iguais). Se ao lançarmos 100 vezes, essa mesma moeda obtivermos 8 caras, teremos mais evidências para aceitar a proposição de que seja viciada, mas ainda assim não poderemos afirmar que a proposição seja verdadeira. Proposições desse tipo que envolvem um grau de confiabilidade da sua veracidade são propsições de natureza estatística.
