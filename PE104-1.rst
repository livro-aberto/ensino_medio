...
Suponha que na primeira segunda-feira do mês de março de 2018, um posto de saúde tenha registrado as idades (em anos completos) das seis primeiras pessoas que chegaram para tomar a vacina da febre amarela e, os registros, obtidos foram `\{55, 22, 30, 14, 25, 40\}`. Neste exemplo dizemos que o número de observações, denotado por `n` é dado por `n=6` e que as observações são dadas por `x_1=55`, `x_2=22`, `x_3=30`, `x_4=14`, `x_5=25` e `x_6=40`.
...
Os valores do conjunto não estão necessariamente ordenados do menor para o maior, por exemplo, no exemplo das idades das pessoas que chegaram para tomar a vacina, `x_1=55`, `x_2=22` e `x_3=30` de modo que `x_1>x_2` e `x_2<x_3`.

Podemos também usar uma notação para representar os dados ordenados. 
...
Por exemplo, no conjunto das seis primeiras pessoas que chegaram para tomar a vacina da febre amarela os registros obtidos foram `\{55, 22, 30, 14, 25, 40\}` tal que `x_1=55`, `x_2=22`, `x_3=30`, `x_4=14`, `x_5=25` e `x_6=40`.
...
Observe que no exemplo das idades das seis primeiras pessoas que chegaram para vacinar temos
...
O cálculo da média já é conhecido desde o Ensino Fundamental e consiste na soma dos valores do conjunto dividida pelo número de observações. Por exemplo, no exemplo das idades das seis primeiras pessoas que chegaram para vacinar, a soma das idades é 186 tal que a média será dada por `\frac{186}{6}=31` anos.
...
fornecendo a expressão que define a média.
...
Por exemplo, considerando os dados do exemplo das idades das seis primeiras pessoas que chegaram para vacinar a média é 31 anos, porém não se observou uma idade igual a 31 anos.
...
Cuidado com esta interpretação: o ponto de equilíbrio corresponde a posição para a qual a soma dos valores, interpretada como peso, é a mesma à esquerda e à direita dela. Esta posição não é necessariamente a posição na qual a área total do histograma é dividida em duas metades (mediana). É claro que, se a forma do histograma for simétrica, estas duas posições serão coincidentes.

.. incluir figura simétrica
...
No exemplo a seguir, foram registrados os dados sobre renda em salários mínimos de 10 clientes de um supermercado: `\{1, 1, 2, 3, 4, 5, 5, 6, 9, 10\}`. 

A média destes dados é `\bar{x}=\frac{46}{10}=4,6`, que representa bem este conjunto, pois nele existem cinco valores acima da média e cinco valores abaixo da média que não estão muito afastados do valor da média 4,6. 

Suponha, por exemplo, que a pessoa que registrou estes dados tenha apertado com mais força no zero na hora de digitar 10 e no valor registrado ficou em 100 tal que o conjunto digitado foi `\{1, 1, 2, 3, 4, 5, 5, 6, 9, 100\}`.  Observe que a única diferença entre o conjunto real e o errado está no valor extremo que deveria ser 10 e ficou como 100. O que este erro acarreta na média?
Com os dados errados a média é dada por `\frac{136}{10}=13,6`, valor bem maior do que a maioria dos dados observados no conjunto, a saber, apenas uma observação é superior a 13,6. Isso mostra que em presença de dados atipicamente altos, deve-se tomar cuidado em escolher a média como medida de posição das observações coletadas. Uma medida pouco afetada para valores atípicos, conhecida como :index:`medida robusta<medida robusta>`,  deverá ser considerada em situações deste tipo. A mediana, que trataremos a seguir, é considerada uma medida robusta.
