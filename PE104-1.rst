...
Por exemplo, considerando novamente os dados das notas de Artes antes da bonificação, vimos que ao todo são 35 notas variando de 0,8 até 8,0 e, a média resultante, foi 5,9. Observe que 5,9, a média da turma, é um valor entre 0,8 (menor nota da turma) e 8,0 (maior nota da turma), porém não se observaram nas notas de Artes desta turma a nota 5,9.
...
Os pontos médios das classes são 1, 3, 5 e 7 e, as frequências absolutas, 1, 5, 6 e 23, respectivamente. A média correspondente a este agrupamento, é dada então por
   ...
   `\textsf{média}=\bar{x}=\frac{n_1\cdot x_{m1}+n_2\cdot x_{m2}+\cdots+n_c\cdot x_{mc}}{n}`
   
   Introduzindo o valor `n` para dentro do somatório, obtemos
   
   `\textsf{média}=\bar{x}=f_1\cdot x_{m1}+f_2\cdot x_{m2}+\cdots +f_c\cdot x_{mc}` 
