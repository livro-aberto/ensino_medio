...
`\textsf{média}=\bar{x}=\frac{n_1\cdot x_{1}+n_2\cdot x_{2}+\cdots+n_c\cdot x_{v}}{n_1+n_2+\cdots n_c}`
...
.. admonition:: Observação 1 
 ...
 Como calcular a média quando os dados estão agrupados em `c` intervalos de classe? 
   ...
   Se os dados são apresentados em classes com as respectivas frequências da classe, a fórmula anterior também expressa a média dos dados, com `x_i` agora representando o ponto médio do intervalo da `i`-ésima classe e `n_i` a frequência absoluta da `i`-ésima classe.
