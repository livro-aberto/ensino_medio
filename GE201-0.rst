...
.. note:: Enunciado e prova da versão acima do Teorema de Tales. A prova será por áreas.

**Uma reta paralela a (e diferente de) um dos lados de um triângulo, determina nos outros dois lados segmentos proporcionais.**

Afirmação equivalente: Dado um triângulo `ABC` e `r` reta paralela a `BC`, que intersecta `AB` e `AC` em `X` e `Y`, respectivamente, então `\dfrac{AX}{XB} = \dfrac{AY}{YC}`.
