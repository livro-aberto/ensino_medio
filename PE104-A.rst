...
O gráfico construído na :ref:`ativ-construcao-do-boxplot` é uma versão simplificada do gráfico conehcido como **boxplot** na qual não se consideram valores atípicos. Este gráfico, muito simples de ser construído, usa a informação das medidas Mínimo, Q1, Mediana, Q3 e Máximo.

Antes de apresentar a explicação detalhada da contrução de um **boxplot** vamos responder a pergunta: "Para que servem os quartis da distribuição?"
