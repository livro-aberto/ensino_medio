...
A inflação pode ser medida de várias formas diferentes. O índice oficial de inflação no Brasil é o IPCA (Índice de Preços ao Consumidor Amplo), que mede a variação mensal de preços de produtos considerando o consumo de famílias com renda mensal entre 1 e 40 salários mínimos. O IBGE (Instituto Brasileiro de Geografia e Estatística) é o orgão responsável pela medição e divulgação do IPCA. Veja neste 
...
Como foi visto na atividade anterior, o valor da variância (desvio padrão) é relativo ao conjunto de dados, de modo que não é possível afirmar que um valor 100 de variância seja muito grande ou pequeno. Isso vai depender dos valores observados no conjunto de dados. Na atividade anterior, estudamos dois conjuntos de dados que apresentam médias diferentes, mas suas variâncias são iguais. Podemos dizer que o impacto da variância em relação à  média é o mesmo para os dois conjuntos? 
...
.. math::
   ...
   CVA=\frac{\overbrace{s}^{\textsf{desvio padrão amostral}}}{\underbrace{\bar{x}}_{\textsf{média amostral}}}\cdot 100 \%
...
.. math::
   ...
   CVP=\frac{\overbrace{\sigma}^{\textsf{desvio padrão populacional}}}{\textsf{média populacional}}\cdot 100\%
