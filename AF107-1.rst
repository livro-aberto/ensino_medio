...
Concluimos assim que o gráfico da função `f` é obtido deslocando-se o gráfico da função linear `d(t)=2t` uma unidade na vertical para cima. Por se tratarem de retas paralelas podemos também dizer que `2` é a taxa de variação de `f`.
...
.. admonition:: Observação
   ...
   Toda função real de variável real que pode ser representada na forma `f(x)=ax+b` é chamada de função afim. O número `a` é chamado de taxa de variação de `f` e `b` é a medida do deslocamento que se faz a partir da função linear `\ell(x)=ax` para se obter o gráfico de `f`.
...
Se considerarmos uma reta no plano e a função afim `f` que a tem como gráfico, a inclinação da reta coincide com a taxa de variação da função `f`. Por exemplo, a reta da Figura `2.7` acima é o gráfico da função afim `f(x)=\dfrac 54 x +b`. Para determinarmos o valor de `b`, basta substituirmos um dos pontos que estão sobre a reta. Como `(1,2)` pertence ao gráfico de `f`, podemos afirmar que `f(1)=2`, logo
   
.. math::

   2=\dfrac 54 \cdot 1 + b \Longrightarrow b=2-\dfrac 54 = \dfrac 34

Isto é, a função `f` é dada pela expressão `f(x)=\dfrac 54 x + \dfrac 34`. Já a reta da Figura `2.8` tem expressão `f(x)=-3x`.
