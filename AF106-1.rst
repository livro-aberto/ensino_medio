...
.. admonition:: Para o professor
   ...
   Nessa seção são propostas três atividades: a primeira visa explorar a definição de função de um ponto de vista um pouco mais abstrato, a segunda trata da determinação do conjunto imagem de funções reais e a terceira investiga sob que condições podemos afirmar que duas funções são iguais.
...
.. admonition:: Resposta 
   ...
   #. Ao escolhermos uma pessoa no conjunto `\mathcal{P}` para tentarmos associá-la ao seu irmão três coisas poderão ocorrer: ela não possui irmão, ela possui um único irmão ou ela possui mais de um irmão. Caso ela não tenha irmão teremos elemento do domínio sem imagem e caso ela tenha mais de um irmão haverá ambiguidade na determinação da imagem. Por isso a relação não define uma função.
